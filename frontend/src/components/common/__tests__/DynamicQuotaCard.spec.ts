...
describe('upstream cycle display', () => {
  it('separates an overdue allocation from the next milestone and displays both bounds in compact mode', () => {
    ...
    expect(wrapper.get('[data-testid=dynamic-remaining]').text()).toContain('下次分配 · 上游 50%')
  ...
  it('separates adjustment from reset and never fabricates a local seven-day countdown', async () => {
    ...
    expect(wrapper.text()).toContain('最近分配')
    ...
    expect(stage.get('[data-testid=dynamic-remaining]').text()).toContain('下次分配')
    expect(stage.get('[data-testid=dynamic-remaining]').find('time').exists()).toBe(false) // Usage milestones have no known date.
    ...
    expect(wrapper.get('[data-testid=dynamic-allocation-stage]').text()).toContain('下次分配')
  })
})
