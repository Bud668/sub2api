...
describe('upstream cycle display', () => {
  it('shows the latest quota change and in-flight reserve in both themes', async () => {
    ...
    for (const dark of [false, true]) {
      ...
      expect(wrapper.get('[data-testid=dynamic-cap]').text()).toBe('分配上限 · $600.00')
      expect(wrapper.get('[data-testid=dynamic-cap]').classes()).toEqual(expect.arrayContaining(['bg-primary-50', 'dark:bg-primary-900/30']))
      ...
      expect(wrapper.get('[data-testid=dynamic-start] dt').text()).toContain('本周期起点#1')
      expect(wrapper.get('[data-testid=dynamic-cycle]').text()).toBe('#1')
      expect(wrapper.get('[data-testid=dynamic-quota-meta]').classes()).toContain('text-center')
