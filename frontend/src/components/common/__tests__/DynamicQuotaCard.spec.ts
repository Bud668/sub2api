...
describe('upstream cycle display', () => {
  it('shows the latest quota change and in-flight reserve in both themes', async () => {
    ...
    for (const dark of [false, true]) {
      ...
      expect(wrapper.get('[data-testid=dynamic-allocation-stage]').findAll(':scope > div').map(node => node.attributes('data-testid'))).toEqual(['dynamic-allocated', 'dynamic-used', 'dynamic-remaining'])
      expect(wrapper.get('[data-testid=dynamic-cap]').text()).toBe('上限 · $600.00')
      expect(wrapper.get('[data-testid=dynamic-cap]').classes()).toEqual(expect.arrayContaining(['text-gray-600', 'dark:text-gray-300']))
      expect(wrapper.get('[data-testid=dynamic-cap]').classes()).not.toContain('bg-amber-50')
      ...
      expect(wrapper.get('[data-testid=dynamic-start] dt').text()).toBe('本周期起点')
      expect(wrapper.find('[data-testid=dynamic-cycle]').exists()).toBe(false)
      expect(wrapper.find('details').exists()).toBe(false)
      await wrapper.setProps({ quota: { ...quota, last_change_usd: -20 } })
      expect(delta.text()).toBe('−$20.00')
      expect(delta.attributes('aria-label')).toBe('额度减少 $20.00')
      expect(delta.classes()).toEqual(expect.arrayContaining(['bg-red-50', 'dark:bg-red-950/50']))
  ...
  it.each([false, true])('shows the upstream reset outside details and follows refreshed data (compact=%s)', async compact => {
    ...
    expect(reset.element.parentElement?.lastElementChild).toBe(reset.element)
    expect(wrapper.get('[data-testid="dynamic-confirmed"]').element.nextElementSibling).toBe(reset.element)
  ...
  it('separates adjustment from reset and never fabricates a local seven-day countdown', async () => {
    ...
    expect(wrapper.get('[data-testid=dynamic-cap]').text()).toBe('上限 · $200.00')
    expect(wrapper.text()).toContain('下调保护')
    expect(wrapper.find('details').exists()).toBe(false)
    expect(wrapper.get('[data-testid=dynamic-allocated] dt > span').text()).toBe('当前动态额度')
    expect(wrapper.text()).toContain('$200.00')
    expect(wrapper.get('[data-testid=dynamic-quota-meta]').attributes('title')).toBeUndefined()
