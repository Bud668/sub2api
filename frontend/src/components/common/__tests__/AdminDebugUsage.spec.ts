...
describe('administrator debug weekly summary', () => {
  it('shows weekly usage without monthly totals, dynamic allocation or invented available amounts', async () => {
    ...
    const subscription = { weekly_usage_usd: 0.05, monthly_usage_usd: 94.70, group }
    ...
    expect(wrapper.text()).toContain('原有限额仍生效')
    ...
    await wrapper.setProps({ subscription: { ...subscription, group: { ...group, weekly_limit_usd: null } } })
