  ...
  <section class="debug-usage" data-testid="admin-debug-usage" :aria-label="t('dynamicQuota.adminDebug')">
    <dl class="debug-summary grid grid-cols-2 gap-4 rounded-xl bg-white p-3 tabular-nums dark:bg-dark-800/60">
      ...
      <div class="min-w-0">
        ...
        <dd class="debug-amount mt-1 break-words text-lg font-semibold tracking-tight text-gray-900 dark:text-gray-100">{{ limit > 0 ? formatCurrency(limit) : t('userSubscriptions.unlimited') }}</dd>
      </div>
      <div v-if="$slots.reset" class="debug-reset col-span-2 min-w-0">
        <dt class="text-xs font-medium text-gray-600 dark:text-gray-300">{{ t('dynamicQuota.debugWeeklyReset') }}</dt>
        <dd class="mt-2 text-sm font-medium text-gray-700 dark:text-gray-200"><slot name="reset" /></dd>
      </div>
    </dl>
    <div v-if="limit > 0" class="my-3 h-2 overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600" role="progressbar" :aria-label="t('dynamicQuota.debugWeeklyUsed')" :aria-valuenow="percentage" aria-valuemin="0" aria-valuemax="100">
...
const props = defineProps<{ subscription: Pick<UserSubscription, 'weekly_usage_usd' | 'group'> }>()
const { t } = useI18n()
const limit = computed(() => props.subscription.group?.weekly_limit_usd ?? 0)
const percentage = computed(() => {
  const value = limit.value > 0 ? (props.subscription.weekly_usage_usd || 0) / limit.value * 100 : 0
