  ...
  <section class="quota-card rounded-xl border border-primary-200 bg-transparent p-4 text-left text-sm dark:border-primary-900" :aria-label="t('dynamicQuota.title')">
    <div class="quota-main">
      ...
      <dl class="quota-amounts grid grid-cols-3 gap-4 rounded-xl bg-white p-3 tabular-nums dark:bg-dark-800/60" data-testid="dynamic-allocation-stage">
        <div class="min-w-0 text-center" data-testid="dynamic-allocated">
          <dt class="flex min-w-0 flex-wrap items-center justify-center gap-1.5 text-xs font-semibold text-gray-700 dark:text-gray-200" data-testid="dynamic-bounds">
            <span>{{ t('dynamicQuota.limit') }}</span>
            <span class="inline-flex shrink-0 items-center rounded-md bg-primary-50 px-1.5 py-0.5 text-[10px] font-medium leading-4 text-primary-700 ring-1 ring-inset ring-primary-200 dark:bg-primary-900/30 dark:text-primary-200 dark:ring-primary-800" data-testid="dynamic-cap">{{ t('dynamicQuota.cap') }} · {{ usd(quota.max_limit_usd) }}</span>
          ...
          <dd class="quota-amount mt-1 flex min-w-0 flex-wrap items-start justify-center gap-1.5 text-lg font-bold tracking-tight text-gray-900 dark:text-gray-100">
        ...
        <div class="min-w-0 text-center" data-testid="dynamic-remaining">
    ...
    <dl class="quota-meta mt-3 grid grid-cols-2 gap-x-4 gap-y-2 border-t border-gray-200 pt-3 text-center text-xs dark:border-dark-600" data-testid="dynamic-quota-meta" :title="t('dynamicQuota.cycleHint')">
      <div class="min-w-0" data-testid="dynamic-start"><dt class="flex flex-wrap items-center justify-center gap-1.5 text-gray-500 dark:text-gray-400"><span>{{ t('dynamicQuota.started') }}</span><span class="rounded bg-gray-100 px-1.5 py-0.5 font-semibold tabular-nums text-gray-700 dark:bg-dark-700 dark:text-gray-200" data-testid="dynamic-cycle">#{{ quota.cycle }}</span></dt><dd class="mt-0.5 break-words font-medium tabular-nums text-gray-900 dark:text-gray-100">{{ date(quota.started_at) }}</dd></div>
