...
const initial = (): DynamicQuotaAdminStatus => ({
  ...
  sources: [{ id: 4, name: 'Test source', pool_settings: { revision: 0, usage_ceiling_percent: 98 } }, { id: 5, name: 'Other source', pool_settings: { revision: 2, usage_ceiling_percent: 95 } }]
})
...
describe('dynamic quota settings', () => {
  ...
  it('defaults OFF; saves explicit source, keeps the dialog open and reloads the server revision', async () => {
    ...
    expect((wrapper.get('#dynamic-usage-ceiling').element as HTMLInputElement).value).toBe('98')
    await wrapper.get('#dynamic-increase-threshold').setValue('5')
    await wrapper.get('#dynamic-usage-ceiling').setValue(95.25)
    await wrapper.get('input[type=checkbox]').setValue(true)
    const response = initial()
    response.policy = { ...response.policy, enabled: true, account_id: 4, revision: 1, increase_threshold_usd: 5, pool_settings: { revision: 1, usage_ceiling_percent: 95.25 }, cycle: 1, status: 'learning', used_usd: 20, limit_usd: 500, remaining_usd: 480 }
    ...
    expect(save).toHaveBeenCalledWith(11, { enabled: true, account_id: 4, revision: 0, weight: 1, max_limit_usd: 700, increase_threshold_usd: 5, pool_settings: { revision: 0, usage_ceiling_percent: 95.25 } })
    expect(wrapper.get('[data-testid=dynamic-save]').attributes('disabled')).toBeDefined()
    ...
    expect((wrapper.get('#dynamic-usage-ceiling').element as HTMLInputElement).value).toBe('95.25')
    ...
    expect(save.mock.calls[1][1].pool_settings.revision).toBe(1)
  })
  ...
  it('keeps unsaved inputs and reports a stale-save conflict without closing', async () => {
    ...
    expect((wrapper.get('#dynamic-usage-ceiling').element as HTMLInputElement).value).toBe('95')
    await wrapper.get('#dynamic-usage-ceiling').setValue(90)
    ...
    expect((wrapper.get('#dynamic-usage-ceiling').element as HTMLInputElement).value).toBe('90')
    expect(save.mock.calls[0][1].pool_settings.revision).toBe(2)
    expect(wrapper.emitted('close')).toBeUndefined()
