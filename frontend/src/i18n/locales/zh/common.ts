export default {
  dynamicQuota: {
    ...
    thresholdHint: '上调默认 $10，可选 $5；下调累计 $5。以上次已生效额度为基准比较，上调每 30 分钟检查、下调每次同步检查。安全保护不受显示门槛限制。首次分配、确认重置或手动改变分配配置立即重算。',
    ...
    learningHint: '当前是临时分配，尚未测出上游总额度。初始基础份额占预算的 80%，其余按需求分配；个人上限不等于已分配额度。',
