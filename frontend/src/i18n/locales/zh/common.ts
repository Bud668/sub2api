export default {
  dynamicQuota: {
    ...
    unsupportedPlatform: '动态订阅目前支持 OpenAI；此平台使用余额计费。',
    resetShort: '下次上游重置（预计）',
    groupHint: '在分组统一设置，现有及新增普通订阅自动继承。按固定名额预留份额，空位不借给其他成员；生效后替代日／周／月额度，保留已用和在途。管理员调试不领取份额，转为调试前占用的名额本周期仍保留。',
    groupMembers: '参与分配 {members} 人 · 管理员调试 {debug} 人',
    legacyMembers: '现有 {n} 条独立配置。保存分组后统一应用本页参数，保留已用量与周期；来源不一致时整批不保存。',
    adminDebug: '管理员调试订阅', adminDebugHint: '仅适用于管理员账号。不领取动态份额，实际消耗仍影响上游余量。使用独立周额度，不受分组日／周／月金额限制；有效期、模型和安全限制仍生效。',
    debugWeeklyUsed: '本周已用', debugWeeklyLimit: '周额度', debugWeeklyReset: '下次周重置',
    debugUsageHint: '独立周额度 · 不参与动态分配',
    debugRemainingHint: '独立周额度减去本周已用和在途预占，不代表上游保证可用量。等待旧请求结算并同步重置时暂不可用；缺少余额数据时显示 —。',
    setDebugWeeklyLimit: '设置周额度',
    debugQuotaHint: '仅修改此管理员调试订阅的周额度，不清已用量，不改变普通成员的名额。分组绑定上游后，周重置跟随该来源的真实重置确认；临时切换测试账号不会更换重置来源。',
    debugQuotaZero: '填 0 表示不限周金额。初始保留原周限额一次，之后独立设置。',
    debugQuotaSaved: '周额度保存成功，已用量保持不变。',
    debugQuotaFailed: '读取失败或保存结果未确认，输入已保留。请刷新核对后再保存。',
    debugResetPending: '等待旧请求结算后同步重置',
    debugResetExpected: '预计 {time} · 以上游确认为准',
    adminDebugOnly: '调试订阅仅能分配给管理员账号，请关闭此开关或选择管理员。',
    enableAdminDebug: '转为管理员调试',
    enableAdminDebugConfirm: '将 {user} 的此订阅转为管理员调试？转换后使用独立周额度，不再领取动态额度；原已占名额保留到对应上游确认重置。已用量、历史账务和在途结算保留，实际消耗仍计入上游。有效期、模型和安全限制继续生效。',
    adminDebugEnabled: '已转为管理员调试，历史用量和账务已保留。',
    adminDebugFailed: '转换失败，请刷新后重试。',
    manualReset: '手动同步重置', checkReset: '校验并同步',
    manualResetHint: '重新校验此上游的真实重置状态。确认后同步绑定此来源的动态订阅；其他上游不受影响。旧周期账务按原规则结算或归档，不追扣到新周期。',
    manualResetScope: '上游 #{source} · 当前周期 #{cycle} · 影响 {members} 个动态订阅',
    manualResetFailed: '暂时无法校验，请稍后重试。不会因校验失败清空用量。',
    manualResetResult: {
      reset: '已确认上游重置，动态周期已同步；重复点击不会再次补发额度。',
      confirming: '已检测到重置信号，等待第二次独立确认。30 秒后可再次校验，或等待后台自动完成（通常 1～2 分钟）。',
      unchanged: '未检测到新的上游重置，当前额度和已用量保持不变。',
      unconfirmed: '上游重置证据尚未通过校验，当前用量保留。请核对上游状态后重试。'
    },
    initialAllocatedAt: '首次分配', notAllocated: '尚未自动分配', nextStage: '下次分配节点',
    stageAt: '这里的 {percent}% 指绑定上游账号的 7 日总用量。达到此节点且数据校验通过后，系统按分组规则重新分配当前额度，可能上调或下调；已用量不会清零，剩余可用随之更新。触发时间取决于实际消耗。', noNextStage: '本周期暂无下一节点',
    stageShort: '下次调额 · {percent}%', noNextStageShort: '暂无下一节点',
    learningStageShort: '学习校验 · {percent}%',
    learningCheckHint: '有效样本 {samples}/{required}。这里的 {percent}% 指绑定上游账号的 7 日总用量；达到节点后检查是否满足调额条件，不保证立即调额或金额变化。样本稳定后恢复分阶段调额，已用量不会清零。',
    capacityCheckHint: '额度变化待确认，验证样本 {samples}/{required}。绑定上游账号的 7 日总用量达到 {percent}% 后重新校验，不保证立即调额或金额变化；校验通过前不按异常估算扩额，已用量不会清零。',
    ...
    slotsHelp: '名额由管理员设置，不随用户加入或退出自动变化。空名额每个周期都持续预留；已占名额（包括退出者）须等对应上游确认重置才释放。中途改人数需有可信容量，校验通过后立即重算当前分配，不清消费。',
    currentSeats: '当前生效 {slots} 个名额 · 本周期已占 {occupied} 个',
    seatChangeHint: '保存后将调整本周期名额并立即重算分配，其他人的当前额度可能变化。只能删除未占用的空位；若已有消费和在途超过新份额，整次修改不生效。',
    slotsInvalid: '请填写 1～1000 的整数名额数。',
    slotsFull: '名额不足：不能少于有效成员或本周期已占名额；退出者的名额本周期仍保留。',
    seatRetained: '此用户本周期的名额仍归原订阅，不能删除再建重新领取；请续期原订阅。',
    seatsSpent: '已有成员的消费和在途超过新均分份额，本周期无法安全调整为此名额数；原配置和账务未改动。',
    seatsLearning: '本周期仍在学习或数据待校验，暂不能安全重分名额；取得至少两个可信区间后可重试，原配置未改动。',
    fixedLearningHint: '尚未测出上游总额度，仅开放有限起步额度，不放满分配上限。只学习本周期；0～10% 每 2 个百分点、10～30% 每 5 个百分点、30% 后每 10 个百分点校验。样本不足或异常时仍按 2 点校验。',
    floorHelp: '下调保护说明', floorHint: '仅用于自动调额，不承诺最低可用量。',
    floorInvalid: '请填写大于 0 且不超过分配上限的下调保护金额。',
    v2AllocationHint: '起步按上限的 10% 计算，最低 $20、最高 $60，且不超过上限；不保证最低可用量。0～10% 每 2 个百分点、10～30% 每 5 个百分点、30% 后每 10 个百分点校验；样本不足或异常时仍按 2 点校验。普通自动上调累计达到 $20 才生效，安全下调立即执行。仅使用本周期证据，保存和开关切换不清用量。',
    v2LearningHint: '尚未测出上游总额度，初始学习期按管理员保存的分配上限执行；达到下一有效节点后重新分配。',
    cardTitle: '动态周额度', details: '查看详情', rules: '规则说明',
    nextAdjustment: '下次调额：上游用量达到 {percent}%', allocatedAt: '最近调额',
    accountingIndependent: '未结账务独立处理中，不阻止合法配置保存；查看订阅管理中的账务明细。',
    pendingActivation: '配置已保存，等待来源数据生效；生效前继续使用原规则。',
    fixedPendingActivation: '配置已保存，等待可信来源数据；普通动态订阅暂不放行新请求，已有请求继续结算。',
    ...
    cycle: '订阅额度周期', used: '本周期已用', limit: '当前动态额度', remaining: '剩余可用', reserved: '在途预占',
    synced: '最近数据同步', confirmed: '最近重置确认', notReset: '尚未确认重置', expected: '下次上游重置（预计）',
    resetUnavailable: '等待上游同步', resetHint: '跟随绑定上游。这里是下一次预计重置时间，不表示上一次重置未完成；实际重置确认后才开始新周期。',
