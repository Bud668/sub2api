export default {
  dynamicQuota: {
    ...
    absorption: {
      display: '显示', uncleared: '未清理', cleared: '已清理', all: '全部记录',
      selectPage: '全选本页', clearSelected: '清理所选（{n}）', clearedAt: '已清理 · {time}',
      clearSuccess: '已清理 {n} 条，可在「已清理」查看；账务和审计记录未删除。',
      clearFailed: '清理结果未确认，所选记录已保留。请刷新核对后重试，不影响账务。',
      standardAmount: '已确认标准费用', smallException: '小额异常，已自动由站点承担',
      automaticClosed: '已结案 · 未向用户计费',
      automaticUnmetered: '请求中断后未取得完整上游计量，已自动结案，不按预占估算扣款；保留审计记录，已结束的预占不再占用额度。',
      ...
      failed: '读取失败，请刷新重试', hint: '以下均为已处理记录，无需手工核销，不再追加扣款。默认统计当前筛选下全部未清理记录，不限本页。清理仅隐藏提醒，可切换到「已清理」查看，不删除账务或计费凭证，不产生扣款、退款或额度重置。无法核实的金额不按零计算；原预占仅供参考，不是实际账单。上游确认重置后，旧周期记录归入历史。',
    ...
    seatsLearning: '预计总费用暂不可用或数据待校验，暂不能安全重分名额；数据恢复后可重试，原配置未改动。',
    fixedLearningHint: '尚未达到首个 2% 节点或暂无有效预计总费用，仅开放有限起步额度；取得本周期估算后按固定节点分配，不再等待独立学习样本。',
    floorHelp: '下调保护说明', floorHint: '仅用于自动调额，不承诺最低可用量。',
    floorInvalid: '请填写大于 0 且不超过分配上限的下调保护金额。',
    v2AllocationHint: '暂无有效估算时，起步按上限的 10% 计算，最低 $20、最高 $60，且不超过上限；这不是下限保护。取得本周期预计总费用后，扣除账号实际 7d 阈值保护部分，按固定名额分配；0～10% 每 2 个百分点、10% 后每 5 个百分点调额，空位保留。个人上限始终封顶，不保证最低可用量。上调不设金额门槛，安全下调立即执行，保存不清用量。',
    v2LearningHint: '等待本周期有效预计总费用；初始额度仍受管理员设置的上限约束，取得估算后按固定节点分配。',
