export default {
  dynamicQuota: {
    ...
    v2AllocationHint: '暂无有效估算时，起步按上限的 10% 计算，最低 $20、最高 $60，且不超过上限；这不是下限保护。取得本周期预计总费用后，扣除账号实际 7d 阈值保护部分，按固定名额分配；达到 2% 后每 1 个百分点调额，空位保留。个人上限始终封顶，不保证最低可用量。上调不设金额门槛，安全下调立即执行，保存不清用量。',
    v2LearningHint: '等待本周期有效预计总费用；初始额度仍受管理员设置的上限约束，取得估算后按固定节点分配。',
    cardTitle: '动态周额度', rules: '规则说明',
    ...
    cycle: '订阅额度周期', used: '本周期已用', limit: '当前动态额度', remaining: '剩余可用', reserved: '在途预占', increasedBy: '额度增加 {amount}', decreasedBy: '额度减少 {amount}',
