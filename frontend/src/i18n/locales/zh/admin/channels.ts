export default {
    ...
    riskControl: {
      ...
      cyberPolicyAutoBan: '上游 cyber_policy 首次命中即停用用户',
      cyberPolicyAutoBanHint: '独立规则：仅今后上游明确返回 cyber_policy 时保护性停用普通用户，所有 Key 后续请求被拒绝，管理员可恢复。可能误判；不追溯历史、不因关键词命中执行本规则、不受累计阈值或下方排除计数开关限制。管理员账号保留人工处置，邮件仍需配置 SMTP。',
      cyberPolicyExcludeBan: 'cyber_policy 不计入封号次数',
      cyberPolicyExcludeBanHint: '开启后，cyber_policy 不参与累计阈值封号；不影响上方独立的首次命中停用规则。历史累计亦排除，风控日志与通知邮件照常。',
