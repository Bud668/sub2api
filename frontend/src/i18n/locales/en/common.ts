export default {
  dynamicQuota: {
    ...
    absorption: {
      reviewTitle: 'Needs review', standardAmount: 'Verified standard cost',
      reviewHint: 'These exceptions remain after recovery and do not block quota saves. Totals use standard costs; the charge button shows the original customer price. State and original cycle are rechecked on confirmation. Holds cannot price an unknown bill; site coverage is final.',
      reviewThreshold: 'Exception review threshold reached', smallException: 'Small exception automatically covered by the site',
      receiptScope: 'Receipt owner or billing type needs review',
      charge: 'Charge', cover: 'Site cover', cannotCharge: 'A complete receipt in the original cycle is required. Holds are reference estimates only.',
      confirmCharge: 'Charge the original {amount} to {email}? Already-billed usage is not charged again, and old-cycle bills cannot enter a new cycle.',
      confirmCover: 'Cover this exception for {email}? No refund is issued and this record cannot be charged afterward.',
      actionSuccess: 'Resolved. Accounting details have been updated.',
      actionFailed: 'The action could not be confirmed. Refresh and verify its state before retrying; holds are never used as prices.',
