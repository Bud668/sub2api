export default {
  dynamicQuota: {
    ...
    absorption: {
      display: 'Show', uncleared: 'Uncleared', cleared: 'Cleared', all: 'All records',
      selectPage: 'Select this page', clearSelected: 'Clear selected ({n})', clearedAt: 'Cleared · {time}',
      clearSuccess: 'Cleared {n} records. View them under Cleared; billing and audit records were not deleted.',
      clearFailed: 'Clear result unconfirmed; selection retained. Refresh to verify before retrying. Billing is unchanged.',
      ...
      automaticUnmetered: 'The interrupted request had no complete upstream metering. Automatically closed without charging an estimate; audit history remains and the finished hold is released.',
      ...
      failed: 'Unable to load; refresh to retry', hint: 'These records are already resolved. No manual write-off or additional charge is needed. Totals default to all matching uncleared records, not just this page. Clearing only hides notifications; Cleared still shows them. It never deletes billing evidence, charges, refunds or resets quota. Unverifiable amounts are not zero; holds are references, not bills. Confirmed upstream resets move old cycles to history.',
    ...
    seatsLearning: 'The estimate is unavailable or awaiting verification. Retry when valid data returns; current settings are unchanged.',
    fixedLearningHint: 'Before the first 2% milestone or a valid current-cycle estimate, only a bounded startup allowance is available. Allocation uses fixed milestones without independent learning samples.',
    ...
    v2AllocationHint: 'Only before a valid estimate, startup uses 10% of the cap, between $20 and $60 and never above the cap; this is not a floor. Share this cycle’s usage-window estimate after excluding the actual 7d threshold reserve. Allocate every 2 points through 10%, then every 5 points, retaining vacant seats. Personal caps always apply; no minimum is guaranteed. Increases have no dollar gate; safety decreases apply immediately. Saving preserves usage.',
    v2LearningHint: 'Waiting for a valid current-cycle estimate. Initial allowance remains bounded by the saved cap; fixed milestones allocate once an estimate is available.',
