...
async function checkCard(page, card, dark, locale) {
  ...
  assert.equal(await card.getByTestId('dynamic-cap').innerText(), locale === 'zh' ? '上限 · $600.00' : 'Cap · $600.00')
  assert.deepEqual(await card.getByTestId('dynamic-cap').evaluate(el => { const s = getComputedStyle(el); return [s.backgroundColor, s.color, s.fontSize] }), ['rgba(0, 0, 0, 0)', dark ? 'rgb(209, 213, 219)' : 'rgb(75, 85, 99)', '11px'])
  assert.notEqual(await card.getByTestId('dynamic-reserved').evaluate(el => getComputedStyle(el).backgroundColor), 'rgba(0, 0, 0, 0)')
  assert.deepEqual(await card.locator('.quota-amounts').evaluate(el => { const s = getComputedStyle(el); return [s.paddingLeft, s.paddingRight] }), metrics.columns === 3 ? ['32px', '32px'] : ['12px', '12px'])
