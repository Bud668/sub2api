...
func TestDynamicQuotaCapacityGuard(t *testing.T) {
	...
	require.Equal(t, "quota_paused", p.accessStatus(now.Add(5*time.Minute)))
	...
	require.Equal(t, "quota_paused", p.accessStatus(now.Add(6*time.Minute)), "volatile spikes cannot continually restart the pause counter")
	p.considerCapacity(1500, now.Add(7*time.Minute))
	require.Equal(t, 1500.0, p.CapacityUSD, "downward safety changes do not wait")
	require.Nil(t, p.CapacityReview)
	require.Equal(t, "quota_paused", p.accessStatus(now.Add(7*time.Minute)), "one recovered query must not clear a persistent anomaly")

	// A smaller positive proposal is also a recovery, not permission to reopen.
	...
	require.False(t, p.growthFrozen())
}
