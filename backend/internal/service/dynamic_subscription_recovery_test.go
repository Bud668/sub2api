...
func TestDynamicQuotaRecoveryKeepsHoldsAndSourceOwnership(t *testing.T) {
 ...
 VALUES($1,4,0,101,2)`, legacyID)
	...
	var owned int
	require.NoError(t, db.QueryRow(`SELECT count(*) FROM dynamic_quota_requests WHERE owner_subscription_id=11 AND status='pending'`).Scan(&owned))
...
func TestDynamicQuotaOperatorAbsorptionClosesOnlyCustomerLiability(t *testing.T) {
 ...
 VALUES($1,4,0,103,2,'uncertain',NOW())`, legacyID)
	...
	require.NoError(t, err)
	require.False(t, status.Policy.Enabled)
	require.NoError(t, s.Save(ctx, 12, DynamicSubscriptionInput{Enabled: true, AccountID: 4, Weight: 1, MaxLimitUSD: 700, FloorLimitUSD: dynamicTestFloor()}))
	var owner int64
	require.NoError(t, db.QueryRow(`SELECT status,owner_subscription_id FROM dynamic_quota_requests WHERE id=$1`, other.ID).Scan(&state, &owner))
	require.Equal(t, "pending", state)
	require.EqualValues(t, 12, owner, "saving retains the other customer's live request")
}
