...
// Persisted milestone and abnormal-estimate guard, not a separate learner.
type DynamicQuotaV2State struct {
	LastNode           int       `json:"last_node"`
	CumulativeEstimate bool      `json:"cumulative_estimate,omitempty"`
	...
	CandidatePercent   float64   `json:"candidate_percent,omitempty"`
...
func (p *DynamicQuotaPoolState) startV2() {
	...
	p.V2 = &DynamicQuotaV2State{UnreservedCapacity: true, FixedSeats: fixed, CumulativeEstimate: true}
	if p.Snapshot != nil {
...
// Share the usage-window estimate. Convert account dollars to standard-cost
// units before applying subscription/peak rates; account multipliers must not
// create or destroy members' physical shares.
func (p *DynamicQuotaPoolState) observeWindowEstimate(o DynamicQuotaObservation) {
	if o.UsedPercent < 2 {
		return // Keep the bounded startup allowance until the first fixed milestone.
	}
	estimate := usagestats.EstimateWindowTotalCost(o.WindowCostUSD, o.UsedPercent)
	if estimate == nil || o.WindowStandardUSD <= 0 {
		return
	}
	sample := *estimate * (o.WindowStandardUSD / o.WindowCostUSD)
	if !validDynamicAmount(sample) || sample <= 0 {
		return
	}
	v := p.V2
	if !v.CumulativeEstimate {
		// Handoff preserves spending and grants. Only old non-seat nodes were
		// ten-percent indices; fixed-seat nodes were already percentages.
		if !v.FixedSeats {
			v.LastNode *= 10
		}
		v.CumulativeEstimate = true
		v.CandidateUSD, v.CandidateSamples = 0, 0
		p.LastAllocationAt = time.Time{} // Recalculate the current milestone once.
	}
	if p.CapacityUSD > 0 && (sample < p.CapacityUSD*0.8 || sample > p.CapacityUSD*1.2) {
		if v.CandidateSamples == 0 || math.Abs(sample-v.CandidateUSD) > v.CandidateUSD*0.05 {
			...
			v.CandidateAt, v.CandidatePercent = o.FetchedAt, o.UsedPercent
		} else if o.UsedPercent > v.CandidatePercent && o.FetchedAt.Sub(v.CandidateAt) >= dynamicQuotaConfirmDelay {
			v.CandidateUSD = math.Min(v.CandidateUSD, sample)
			v.CandidateSamples++
			v.CandidateAt, v.CandidatePercent = o.FetchedAt, o.UsedPercent
		}
		if v.CandidateSamples < dynamicQuotaGuardChecks {
			return // Re-reading one percentage cannot authorize a 1800 -> 9000 spike.
		}
		sample = v.CandidateUSD
	}
	v.CandidateUSD, v.CandidateSamples = 0, 0
	v.CandidateAt, v.CandidatePercent = time.Time{}, 0
	p.CapacityUSD = sample
}

func (p *DynamicQuotaPoolState) v2AllocationDue(now time.Time) bool {
	return p.V2 != nil && p.V2.CumulativeEstimate && p.Snapshot != nil && p.Snapshot.Valid(now) &&
		p.Status == "active" && !p.growthFrozen(now) && p.CapacityUSD > 0 && p.Snapshot.hasWindowEstimate() &&
		(dynamicQuotaNode(p.Snapshot.UsedPercent) > p.V2.LastNode || p.LastAllocationAt.IsZero())
}
...
func (s *DynamicSubscriptionService) reallocateV2(ctx context.Context, tx *sql.Tx, accountID int64, p *DynamicQuotaPoolState, now time.Time) error {
 ...
 WHERE p.account_id=$1 AND `+dynamicActiveMemberSQL+` ORDER BY p.subscription_id`, accountID)
	...
	allocations, err := allocateDynamicQuotaV2(members, p.Available(now, total, 0))
 ...
 allocated_standard_usd=$3,updated_at=NOW() WHERE subscription_id=$1 AND account_id=$4`, m.ID, limit, allocation, accountID, dynamicQuotaNode(p.Snapshot.UsedPercent)); err != nil {
			return err
		}
		if limit != q.LimitUSD {
	...
	details, err := json.Marshal(map[string]any{"node": p.V2.LastNode, "changes": changes})
	if err != nil {
		return err
	}
